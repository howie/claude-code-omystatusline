...
- `maxTokens` (denominator) resolution chain (`resolveMaxTokens()`, `cmd/statusline/main.go`; later layers override earlier):
  ...
  2. **Validated `input.ContextWindow.ContextWindowSize`** (`contextWindowFromInput()`): trusted as denominator **only** when it equals a known capacity (200K or 1M) **and is non-demoting** (`inputWindow >= contextWindowForModel()`). It can rescue an under-estimate (an unrecognized 1M model inferred as 200K) but never demotes a true 1M window to 200K — because the field's semantics are not yet verified against a real payload, and demotion is the dangerous `#35/#36` inflated-% direction. The allow-set rejects legacy current-usage values and the model-inference layer remains the fallback — **fail-safe by construction**, never remove `contextWindowForModel()`
